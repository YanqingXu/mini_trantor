...
int main() {
    // Contract 1: HttpCallback fires and response is correct
    // Note: With the default numThreads=1 (TcpServerOptions), the callback
    // fires on the worker thread, not the base loop. This is expected.
    {
        const uint16_t port = allocateTestPort();
        mini::net::EventLoop loop;
        mini::http::HttpServer server(&loop, mini::net::InetAddress(port, true), "contract_http");

        std::promise<void> callbackFired;
        auto callbackFuture = callbackFired.get_future();
        ...
        std::thread timer([&] {
            assert(callbackFuture.wait_for(5s) == std::future_status::ready);
        ...
        client.join();
        timer.join();
        std::printf("  PASS: HttpCallback fires and response is correct\n");
    }
