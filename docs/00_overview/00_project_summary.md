...
4. **协议支持**：TCP（服务端 + 客户端）、TLS/SSL、HTTP/1.1、WebSocket（RFC 6455）、RPC
5. **协程桥接**：C++20/23 coroutine 与 Reactor 调度语义无缝融合 —— `Task<T>`、`CancellationToken`、`SleepAwaitable`、`Timeout`、`WhenAll`/`WhenAny`
6. **DNS 解析**：异步线程池 + TTL 缓存，不阻塞任何 EventLoop 线程
7. **测试体系**：三层分离 —— unit（纯逻辑）、contract（API/生命周期/线程亲和）、integration（端到端）
8. **当前规模**：56 个测试全部通过，覆盖 Reactor、线程模型、协程、DNS、TLS、HTTP、WebSocket、RPC
