...
struct Impl {
    // ===== 线程亲和与身份 =====
    EventLoop* loop;                       // owner loop（ioLoop），不拥有
    std::string name;                      // 唯一名称
    StateE state;                          // kConnecting/kConnected/kDisconnecting/kDisconnected

    // ===== 网络资源 =====
    std::unique_ptr<Socket> socket;        // RAII fd 包装（析构时 close）
    std::unique_ptr<Channel> channel;      // fd 的事件代理（绑定到 ioLoop）
    InetAddress localAddr, peerAddr;

    // ===== 缓冲区 =====
    Buffer inputBuffer;                    // 入站字节缓冲
    Buffer outputBuffer;                   // 出站字节缓冲（write 不完时暂存）
    ...
    // ===== 用户上下文 =====
    std::any context;                      // 用户自定义上下文（透明存储）
};

std::unique_ptr<Impl> impl_;
```

当前实现已经不再把 TLS 状态、awaiter 存储、背压状态直接平铺在 `TcpConnection`
主体中，而是通过 `Impl` 聚合 3 个 loop-owned helper：

- `ConnectionTransport`：封装 plain TCP / TLS 的 non-blocking read/write/handshake/shutdown
- `ConnectionAwaiterRegistry`：封装 read/write/close waiter 的注册、恢复与 close 清理
- `ConnectionBackpressureController`：封装高低水位阈值与读事件暂停/恢复

这让 `TcpConnection` 更接近“连接生命周期中心”，而不是继续演化成一个把所有可选能力都内联进去的神类。
...
```
         ...
         connectEstablished()
               ...
               │ 若启用 TLS：先推进 handshake
               │ 否则 / 握手完成后：connectionCallback_(connected)
               ▼
         [正常收发阶段]
               │
        ┌──────┴──────┐
...
void TcpConnection::handleRead(Timestamp) {
    ...
    if (result.status == Status::kOk) {
        ...
        impl_->callbacks->notifyMessage(shared_from_this(), &impl_->inputBuffer, hasReadWaiter);
        return;
    }

    if (result.status == Status::kPeerClosed) {
        handleClose();                       // 对端关闭
        return;
    }

    if (result.status == Status::kError) {
        handleError(result.savedErrno);      // 读错误 → 同样走 handleClose
    }
}
```

这里的关键变化是：`TcpConnection` 仍然拥有“读事件怎么收敛到生命周期语义”的控制权，
但具体的 transport 差异（plain/TLS）与 awaiter 恢复策略已经分别下沉到
`ConnectionTransport` 和 `ConnectionAwaiterRegistry`。
...
2. await_suspend(handle)：
   ...
   → 内部转到 awaiters_->armReadWaiter(handle, 1024, readyNow)
   → 协程挂起，控制权返回 EventLoop
3. 下次 handleRead 到来：
   → transport_->readInto(...) 填充 inputBuffer
   → awaiters_->resumeReadWaiterIfSatisfied(readableBytes)
   → buffer 满足 minBytes → queueResume(handle)  // 通过 EventLoop 调度
   → 下一轮 loop 中 handle.resume()
4. await_resume()：
   → consumeReadableBytes(minBytes) / PeerClosed → 返回 Expected<string>
```
...
```
      ...
      TcpConnection (ioLoop 线程)
          ...
          ├── ConnectionTransport（plain/TLS 传输细节）
          ├── ConnectionAwaiterRegistry（协程 waiter 状态）
...
| `TcpConnection` → `ConnectionTransport` | `unique_ptr`，封装 plain/TLS I/O 与握手 |
| `TcpConnection` → `ConnectionAwaiterRegistry` | `unique_ptr`，封装 coroutine waiter 状态 |
| `TcpConnection` → `ConnectionBackpressureController` | `unique_ptr`，封装读暂停/恢复策略 |
...
背压策略的核心：通过 `ConnectionBackpressureController` 统一控制读事件订阅，
间接产生 TCP 窗口 0 的背压效果，而不是把 `reading_`、阈值、开关逻辑都散落在 `TcpConnection` 主体中。
...
3. **哪些回调可能重入？**
   `handleRead` 中的 `messageCallback` 可能调用 `conn->send()`，进而同线程进入 `sendInLoop`。`connectionCallback` 在连接建立和断开时都会触发；TLS 开启时，建立回调会延后到握手完成之后。`closeCallback` 由 TcpServer/TcpClient 接管，最终回流到拥有连接映射的 loop。
...
5. **哪个测试文件验证此改动？**
   `tests/contract/tcp_connection/test_tcp_connection.cpp`（read/write/close/awaiter/backpressure 主路径）、`tests/unit/net/test_connection_awaiter_registry.cpp`、`tests/unit/net/test_connection_backpressure_controller.cpp`、`tests/unit/net/test_connection_transport.cpp`（拆分组件单测）。
...
| 未在 connectEstablished 前调用 startTls | `ConnectionTransport` 不会进入握手态，连接将按普通 TCP 路径建立 |
