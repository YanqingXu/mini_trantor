...
mini-trantor 共有 **15 个集成测试文件**，验证完整的端到端场景。
...
| 10 | `coroutine/test_timeout_race.cpp` | timeout 竞态 | 3 | `TimedOut`、peer close、active cancel 三者区分 |
| 11 | `http/test_http_server.cpp` | HTTP 服务器 | 3 | GET 回环、POST body echo、HTTP/1.0 关闭行为 |
| 12 | `ws/test_ws_server.cpp` | WebSocket 服务器 | 3 | 多消息单连接、binary 回环、并发客户端 |
| 13 | `dns/test_dns_client.cpp` | DNS + 连接 | 2 | TcpClient hostname 连接 + echo、协程 resolve + connect 链 |
| 14 | `tls/test_tls_echo.cpp` | TLS echo | 3 | TLS echo 回环、协程 TLS echo、证书验证失败 |
| 15 | `rpc/test_rpc.cpp` | RPC 端到端 | 9 | callback/coroutine call、timeout、error path、coroCall 抛错 |
...
| timeout / close / cancel 竞态 | coroutine/test_timeout_race.cpp |
